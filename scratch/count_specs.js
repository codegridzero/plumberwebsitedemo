...
let output_lines = [];
output_lines.push("==================================================");
output_lines.push("MASTER TEMPLATE CONTENT SPECIFICATION");
output_lines.push("==================================================");
output_lines.push("");
output_lines.push("Template: ProPlumbers Webflow Master Template");
output_lines.push("Source File: index.html");
output_lines.push("");
output_lines.push("Purpose:");
output_lines.push("This document defines the exact content structure and text-length requirements of the Master Template.");
output_lines.push("IMPORTANT: When generating new website content, preserve the same section structure and text-length requirements.");
output_lines.push("");
output_lines.push("--------------------------------------------------");
output_lines.push("");
...
sections_data.forEach(sec => {
    output_lines.push(`## ${sec.name}`);
    output_lines.push("");
    
    let sec_words = 0;
    let sec_chars_inc = 0;
    let sec_chars_exc = 0;
    let sec_elements = sec.elements.length;
    
    sec.elements.forEach((el, idx) => {
        let text = el.text;
        let words = text.trim().split(/\s+/).length;
        let chars_inc = text.length;
        let chars_exc = text.replace(/\s+/g, '').length;
        let lines_count = (text.match(/\n/g) || []).length + 1;
        
        sec_words += words;
        sec_chars_inc += chars_inc;
        sec_chars_exc += chars_exc;
        
        let elem_num = String(idx + 1).padStart(2, '0');
        output_lines.push(`Element ${elem_num}`);
        output_lines.push(`Type: ${el.type}`);
        output_lines.push(`Current Text: ${text}`);
        output_lines.push(`Words: ${words}`);
        output_lines.push(`Characters Including Spaces: ${chars_inc}`);
        output_lines.push(`Characters Excluding Spaces: ${chars_exc}`);
        output_lines.push(lines_count === 1 ? "Lines: 1 line" : `Lines: ${lines_count} lines`);
        output_lines.push(`Requirement: Approximately ${words} words / ${chars_inc} characters including spaces`);
        output_lines.push("");
    });
...
output_lines.push("## SUMMARY TABLE");
output_lines.push("");
output_lines.push("| Section | Number of Text Elements | Total Words | Total Characters |");
output_lines.append ? null : output_lines.push("| ------- | ----------------------- | ----------- | ---------------- |");
summary_rows.forEach(r => {
    output_lines.push(`| ${r.section} | ${r.elements} | ${r.words} | ${r.chars} |`);
});

fs.writeFileSync("master_template_content_specification.md", output_lines.join("\n"), "utf-8");
module.exports = { sections_data };
